...
hostname = 'redis-19111.c264.ap-south-1-1.ec2.cloud.redislabs.com'
portnumber = 19111
password = 'dfl9jNUT8cfbr1oIfymC2VjbXTOWEFK6'
...
def retrive_data(name):
    ...
    try:
        retrieve_df[['Name', 'Role', 'roll_num', 'stu_class']] = retrieve_df['name_role'].str.split('@', expand=True)
    except ValueError:
        print("Invalid data format in the 'name_role' column.")
        return None
    return retrieve_df[['Name', 'Role', 'roll_num', 'stu_class', 'facial_features']]
...
class RegistrationForm:
    ...
    def save_data_in_redis_db(self,name,role,roll_num,stu_class):
        # validation name
        if name is not None:
            if name.strip() != '':
                key = f'{name}@{role}@{roll_num}@{stu_class}'
            else:
                return 'name_false'
        else:
            return 'name_false'
